		...
		items = []mtypes.IPWarmup{
			{
				...
				StageStartTime:   mtypes.RFC3339Time(time.Now()),
...
func (ms *Server) getIPWarmup(w http.ResponseWriter, r *http.Request) {
	toJSON(w, mtypes.IPWarmupDetailsResponse{
		Details: mtypes.IPWarmupDetails{
			...
			StageStartTime:    mtypes.RFC3339Time(time.Now()),
			StageVolumeLimit:  4000,
			StageStartedAt:    mtypes.RFC3339Time(time.Now()),
			HourStartedAt:     mtypes.RFC3339Time(time.Now()),
			PlanStartedAt:     mtypes.RFC3339Time(time.Now()),
			PlanLastUpdatedAt: mtypes.RFC3339Time(time.Now()),
			TotalStages:       15,
			StageHistory: []mtypes.IPWarmupStageHistory{
				{
					FirstUpdatedAt: mtypes.RFC3339Time(time.Now()),
					CompletedAt:    mtypes.RFC3339Time(time.Now()),
					Limit:          1000,
				},
				{
					FirstUpdatedAt: mtypes.RFC3339Time(time.Now()),
