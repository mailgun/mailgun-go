...
func (mg *Impl) GetCredentials(limit, skip int) (int, []Credential, error) {
...
func (mg *Impl) CreateCredential(login, password string) error {
	if (login == "") || (password == "") {
...
func (mg *Impl) ChangeCredentialPassword(id, password string) error {
	if (id == "") || (password == "") {
		return ErrEmptyParam
	}
...
func (mg *Impl) DeleteCredential(id string) error {
	if id == "" {
		return ErrEmptyParam
	}
	r := newHTTPRequest(generateCredentialsURL(mg, id))
